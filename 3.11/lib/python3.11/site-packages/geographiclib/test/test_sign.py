...
class SignTest(unittest.TestCase):
    ...
    def test_AngRound(self):
        ...
        self.assertTrue(SignTest.equiv(Math.AngRound(eps / 32), +eps / 32))
        self.assertTrue(
            SignTest.equiv(Math.AngRound((1 - 2 * eps) / 64), (1 - 2 * eps) / 64)
        )
        ...
        self.assertTrue(
            SignTest.equiv(Math.AngRound((1 - eps / 2) / 16), (1 - eps / 2) / 16)
        )
        ...
        self.assertTrue(
            SignTest.equiv(Math.AngRound((1 - eps / 2) / 8), (1 - eps / 2) / 8)
        )
    ...
    def test_sum(self):
        ...
        s, _ = Math.sum(+9.0, -9.0)
        self.assertTrue(SignTest.equiv(s, +0.0))
        s, _ = Math.sum(-9.0, +9.0)
        self.assertTrue(SignTest.equiv(s, +0.0))
        s, _ = Math.sum(-0.0, +0.0)
        self.assertTrue(SignTest.equiv(s, +0.0))
        s, _ = Math.sum(+0.0, -0.0)
        self.assertTrue(SignTest.equiv(s, +0.0))
        s, _ = Math.sum(-0.0, -0.0)
    ...
    def test_AngDiff(self):
        ...
        s, _ = Math.AngDiff(-0.0, -0.0)
        self.assertTrue(SignTest.equiv(s, +0.0))
        s, _ = Math.AngDiff(+5.0, +365.0)
        self.assertTrue(SignTest.equiv(s, +0.0))
        s, _ = Math.AngDiff(+365.0, +5.0)
        self.assertTrue(SignTest.equiv(s, -0.0))
        s, _ = Math.AngDiff(+5.0, +185.0)
        self.assertTrue(SignTest.equiv(s, +180.0))
        s, _ = Math.AngDiff(+185.0, +5.0)
        self.assertTrue(SignTest.equiv(s, -180.0))
        s, _ = Math.AngDiff(+eps, +180.0)
        self.assertTrue(SignTest.equiv(s, +180.0))
        s, _ = Math.AngDiff(-eps, +180.0)
        self.assertTrue(SignTest.equiv(s, -180.0))
        s, _ = Math.AngDiff(+eps, -180.0)
    ...
    def test_equatorial_NS(self):
        ...
        # lat1 lat2 azi1 azi2
        C = [[+0.0, +0.0, 56, 124], [-0.0, -0.0, 124, 56]]
        for l in C:
    ...
    def test_azimuth_0_180(self):
        ...
        for l in C:
            ...
            direct = Geodesic.WGS84.Direct(
                0.0, 0.0, azi1, 15e6, Geodesic.STANDARD | Geodesic.LONG_UNROLL
            )
