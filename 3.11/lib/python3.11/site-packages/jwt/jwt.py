...
class JWT:
    ...
    def encode(
        ...
        if not (key is None or isinstance(key, AbstractJWKBase)):  # pragma: no cover
            raise TypeError(
                "key must be an instance of a class implements " "jwt.AbstractJWKBase"
            )
        if not (
        ...
        try:
            ...
            raise JWTEncodeError("payload must be able to be encoded to JSON") from why
    ...
    def decode(
        ...
        try:
            ...
            raise JWTDecodeError("a payload of the JWT is not valid JSON") from why
