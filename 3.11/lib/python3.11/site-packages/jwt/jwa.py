...
class AbstractSigningAlgorithm:
    ...
    def verify(
        self, message: bytes, key: Optional[AbstractJWKBase], signature: bytes
    ) -> bool:
        raise NotImplementedError()  # pragma: no cover
...
class RSAAlgorithm(AbstractSigningAlgorithm):
    ...
    def sign(self, message: bytes, key: Optional[AbstractJWKBase]) -> bytes:
        ...
        return key.sign(message, hash_fun=self.hash_fun, padding=padding.PKCS1v15())
    ...
    def verify(
        ...
        key = self._check_key(key)
        return key.verify(
            message, signature, hash_fun=self.hash_fun, padding=padding.PKCS1v15()
        )
