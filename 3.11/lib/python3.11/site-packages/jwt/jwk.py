...
from typing import Any, Callable, Dict, Mapping, Type, TypeVar, Union, Optional
...
class RSAJWK(AbstractJWKBase):
    ...
    def from_dict(cls, dct):
        ...
        if "d" not in dct:
            return cls(pub_numbers.public_key(backend=default_backend()), **dct)
        ...
        if len(product) == 0:
            ...
            raise MalformedJWKError(
                "p, q, dp, dq, qi MUST be present or" "all of them MUST be absent"
            )
...
def jwk_from_bytes_argument_conversion(func: _C) -> _C:
    if not ("private" in func.__name__ or "public" in func.__name__):
        raise Exception(
            "the wrapped function must have either public" " or private in it's name"
        )
...
def jwk_from_public_bytes(
    ...
    *,
    ...
    try:
        ...
        raise UnsupportedKeyTypeError("unsupported key type")  # pragma: no cover
