...
class PandasLikeNamespace:
    ...
    def mean_horizontal(self, *exprs: IntoPandasLikeExpr) -> PandasLikeExpr:
        ...
        def func(df: PandasLikeDataFrame) -> list[PandasLikeSeries]:
            series = (s.fill_null(0) for _expr in parsed_exprs for s in _expr._call(df))
            non_na = (
                1 - s.is_null() for _expr in parsed_exprs for s in _expr._call(df)
            )
            return [
...
class PandasWhen:
    ...
    def __call__(self, df: PandasLikeDataFrame) -> list[PandasLikeSeries]:
        ...
        condition_native = validate_column_comparand(
            value_series_native.index, condition
        )
...
class PandasThen(PandasLikeExpr):
    ...
    def otherwise(
        self, value: PandasLikeExpr | PandasLikeSeries | Any
    ) -> PandasLikeExpr:
