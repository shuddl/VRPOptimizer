...
class Flag(Generic[_T]):
    ...
    def _extra_xml_dom_elements(self, doc: minidom.Document) -> List[minidom.Element]:
...
class MultiFlag(Generic[_T], Flag[List[_T]]):
    ...
    def parse(
        self, arguments: Union[Text, _T, Iterable[_T]]
    ...
    def _parse(
        self, arguments: Union[Text, Optional[Iterable[_T]]]
    ) -> List[_T]:  # pylint: disable=arguments-renamed
        if isinstance(arguments, abc.Iterable) and not isinstance(arguments, str):
    ...
    def _extra_xml_dom_elements(self, doc: minidom.Document) -> List[minidom.Element]:
        ...
        if hasattr(self.parser, "enum_values"):
            for (
                enum_value
            ) in self.parser.enum_values:  # pytype: disable=attribute-error
                elements.append(
                    _helpers.create_xml_dom_element(doc, "enum_value", enum_value)
                )
        return elements
...
class MultiEnumClassFlag(MultiFlag[_ET]):  # pytype: disable=not-indexable
    ...
    def _extra_xml_dom_elements(self, doc: minidom.Document) -> List[minidom.Element]:
        ...
        for (
            ...
            self.parser.enum_class.__members__.keys()
