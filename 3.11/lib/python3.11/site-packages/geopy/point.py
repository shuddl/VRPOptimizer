...
def _normalize_coordinates(latitude, longitude, altitude):
    ...
    if not is_all_finite:
        raise ValueError(
            "Point coordinates must be finite. %r has been passed "
            "as coordinates." % ((latitude, longitude, altitude),)
        )

    if abs(latitude) > 90:
        warnings.warn(
            "Latitude normalization has been prohibited in the newer "
...
class Point:
    ...
    def __new__(cls, latitude=None, longitude=None, altitude=None):
        ...
        latitude, longitude, altitude = _normalize_coordinates(
    ...
    def __setitem__(self, index, value):
        ...
        self.latitude, self.longitude, self.altitude = _normalize_coordinates(*point)
    ...
    def from_sequence(cls, seq):
        ...
        if len(args) > 3:
            raise ValueError(
                "When creating a Point from sequence, it "
