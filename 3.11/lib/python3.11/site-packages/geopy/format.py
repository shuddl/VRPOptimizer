...
DEGREES_FORMAT = "%(degrees)d%(deg)s %(minutes)d%(arcmin)s %(seconds)g%(arcsec)s"
...
def format_degrees(degrees, fmt=DEGREES_FORMAT, symbols=None):
    ...
    format_dict = dict(
        symbols, degrees=degrees, minutes=abs(arcminutes), seconds=abs(arcseconds)
    )
...
ANGLE_DIRECTIONS = {n * 11.25: d for n, d in enumerate(DIRECTIONS)}
ANGLE_DIRECTIONS_ABBR = {n * 11.25: d for n, d in enumerate(DIRECTIONS_ABBR)}
