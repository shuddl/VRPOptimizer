...
class MonitoringSystem:
    ...
    def _setup_metrics(self):
        ...
        self.metrics = {
            ...
            "optimization_count": Counter(
                "optimization_total", "Total number of optimizations"
            ),
            "optimization_errors": Counter(
                "optimization_errors_total", "Total number of optimization errors"
            ),
            "active_optimizations": Gauge(
                "active_optimizations", "Number of currently running optimizations"
            ),
            "memory_usage": Gauge(
                "memory_usage_percent", "System memory usage percentage"
            ),
            "cpu_usage": Gauge("cpu_usage_percent", "System CPU usage percentage"),
            "shipments_processed": Counter(
                "shipments_processed_total", "Total number of shipments processed"
            ),
        }
    ...
    def _check_thresholds(self, metrics: SystemMetrics):
        ...
        if metrics.memory_percent > 90:
            self.logger.warning(
                "High memory usage", memory_percent=metrics.memory_percent
            )
        ...
        if metrics.disk_usage_percent > 90:
            self.logger.warning(
                "High disk usage", disk_percent=metrics.disk_usage_percent
            )
    ...
    async def log_optimization_start(self, num_shipments: int):
        ...
        self.logger.info("Optimization started", num_shipments=num_shipments)
        ...
        try:
            ...
            if format == "json":
                output_file = (
                    ...
                    / f"metrics_{datetime.now():%Y%m%d_%H%M%S}.json"
                )
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    self.settings.SLACK_WEBHOOK_URL, json=payload
                ) as response:
...
if __name__ == "__main__":
    # Run the asynchronous monitor_system function
