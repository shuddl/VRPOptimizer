...
class VisualizationService:
    ...
    def _setup_resource_limits(self):
        ...
        try:
            # Set memory limit to 1GB for visualization processes
            resource.setrlimit(resource.RLIMIT_AS, (1024 * 1024 * 1024, -1))
        except Exception as e:
            self.logger.warning(f"Could not set resource limits: {str(e)}")

    async def create_route_map(self, solution: Solution) -> Tuple[bool, str, Optional[str]]:
    ...
    async def create_timeline(self, solution: Solution) -> Tuple[bool, str, Optional[str]]:
    ...
    async def create_analytics_dashboard(self, solution: Solution) -> Tuple[bool, str, Optional[str]]:
    ...
    def _calculate_map_center(self, solution: Solution) -> Tuple[float, float]:
        ...
        for route in solution.routes:
            for stop_type, shipment in route.stops:
                location = shipment.origin if stop_type == 'pickup' else shipment.destination
    ...
    def _add_route_to_map(self, m: folium.Map, route: Route, color: str):
        ...
        folium.PolyLine(
            ...
            color=color,
            opacity=0.8,
            popup=f"Route {route.id}"
        ).add_to(m)

    def _create_stop_popup(self, route: Route, stop_idx: int, 
                          stop_type: str, shipment: Shipment) -> str:
    ...
    def _prepare_dashboard_data(self, solution: Solution) -> Dict:
        ...
        return {
            ...
            'utilization': [
                {
                    'route_id': route.id,
                    'utilization': (route.total_pallets / self.settings.MAX_PALLETS) * 100
                }
                for route in solution.routes
            ],
