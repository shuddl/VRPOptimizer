...
class CustomConstraintManager:
    ...
    def add_lifo_constraints(
        ...
        try:
            ...
            for request in range(data['num_locations'] // 2):
                ...
                delivery_index = manager.NodeToIndex(request + data['num_locations'] // 2)
        ...
        try:
            ...
            for location_idx, time_window in enumerate(data['time_windows']):
                ...
                time_dimension.CumulVar(index).SetRange(
                    time_window[0],  # start time
                    time_window[1]   # end time
                )

        except Exception as e:
            raise OptimizationError(f"Failed to add time window constraints: {str(e)}")

    def validate_solution(self, solution: List[List[Shipment]]) -> Tuple[bool, Optional[str]]:
        """
                ...
                for route in solution:
                    ...
                    for shipment in route:
                        if shipment.is_pickup:
                            ...
                            if not active_shipments or active_shipments[-1] != shipment.id:
    ...
    def update_config(self, constraint_type: str, enabled: bool = True,
                     weight: Optional[int] = None, penalty: Optional[int] = None) -> None:
        """
