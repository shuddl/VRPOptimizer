...
class SecurityService:
    ...
    def validate_file(self, filename: str, content: bytes) -> Tuple[bool, str]:
        ...
        if len(content) > self.config.MAX_UPLOAD_SIZE:
            return (
                False,
                f"File too large. Maximum size: {self.config.MAX_UPLOAD_SIZE/1024/1024}MB",
            )
...
class SessionManager:
    ...
    def create_session(self, user_id: str) -> str:
        ...
        if len(user_sessions) >= self.config.MAX_SESSIONS_PER_USER:
            ...
            oldest_session = min(
                user_sessions, key=lambda s: self.sessions[s]["created_at"]
            )
        ...
        self.sessions[session_id] = {
            ...
            + timedelta(seconds=self.config.SESSION_LIFETIME),
        }
