...
async def test_optimization_endpoint(test_client, sample_excel_data):
    files = {"file": ("test.xlsx", sample_excel_data, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")}
    ...
    return Solution(
        routes=[
            Route(
                id="R001",
                stops=[
                    ("pickup", test_shipments[0]),
                    ("delivery", test_shipments[0])
                ],
